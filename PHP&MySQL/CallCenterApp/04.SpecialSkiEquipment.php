...
foreach ($stm as $prop) {
    ...
    foreach ($isMountainCountry as $mountain) {
        $currMountainId = $mountain['mountain_id'];
        $mountain = $db->prepare('SELECT * FROM peaks 
                                WHERE mountain_id = :currMountainId AND elevation > 4000;');

        $mountain->bindParam('currMountainId', $currMountainId);
        $mountain->execute();

        echo PHP_EOL;
        echo 'Forward customer for special offers! ';

        foreach ($mountain as $item) {
